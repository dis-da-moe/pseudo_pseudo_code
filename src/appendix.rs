...
macro_rules! expect {
    ($args: expr, $($index: expr => $enum: ident),+) => {
        ($(match &$args[$index] {
            ...
            _ => Err(Execution::IncorrectType(LiteralType::$enum, LiteralType::from(&$args[$index])))
        }),+)
    };
}
...
macro_rules! expect_return {
    ($args: expr, $($index: expr => $enum: ident),+) => {
        ($(match &$args[$index] {
            ...
            _ => return Err(Execution::IncorrectType(LiteralType::$enum, LiteralType::from(&$args[$index])))
        }),+)
    };
}
...
macro_rules! wrong_type {
    ...
    ($expected: ident, $received: expr) => {wrong_type!(LiteralType::$expected, LiteralType::from(&$received))};
...
macro_rules! number_args {
    ($args: expr, $name: expr, $expected_len: expr) => {
        {
            let length = $args.len();
            let fixed_args: [Literal; $expected_len] = 
                $args.try_into()
                .map_err(|_| Execution::IncorrectNumberArguments(String::from($name), $expected_len, length))?;
            fixed_args
        }
    };
}
